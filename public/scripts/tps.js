...
function tpsRequestData(){
    ...
    socket.on("admin-tps-update-tps", (tps) => {
        if(tps){
            ...
            for(id in tps) {
                ...
                let status = !tps[id].status ? 
                    {color: "bg-neutral-300", name: "offline", checked: ""} : !tps[id].logged ? 
                        {color: "bg-orange-300", name:"available", checked: "checked"} : {color: "bg-green-500", name:"ongoing", checked: "checked"};
                ...
                card.innerHTML = `
                    ...
                    <div class="flex items-center">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div class="w-14 h-7 bg-gray-300 rounded-full peer peer-checked:bg-emerald-400 transition-colors"></div>
                            <div class="absolute left-1 top-1 w-5 h-5 bg-white rounded-full transition-all peer-checked:translate-x-7 shadow"></div>
                        </label>
                        <button onclick="deleteTps('${id}')" class="deleteTps inline-flex items-center justify-center rounded-sm w-7 h-7 ml-2 cursor-pointer bg-red-200">
            ...
            lastTps = "TPS-1";
            $id("tps-cards").innerHTML = `
                <div class="p-2 bg-white rounded-sm shadow-md/15 w-[250px] min-w-[240px]">
                    <div class="w-60 h-70 flex flex-col justify-center items-center text-lg text-neutral-600">
                        <p class="text-xs">Tidak ada TPS tersedia</p>
                    </div>
                </div>
            `;
            $id("tps-table").innerHTML = `
                ...
                <!-- table looping by script -->
                <tr class="bg-slate-200 h-15 text-center">
                    <td colspan="4">Tidak ada TPS tersedia</td>
                </tr>
            `;
        }
    });
    ...
    $id("add-tps").addEventListener("click", (e) => {
        Swal.fire({
            ...
            title: 'Nama TPS',
            input: 'text',
            inputLabel: 'Masukkan nama TPS disini',
            inputPlaceholder: 'Contoh: TPS 1, KELAS XI, TPS X MIPA 2',
            showCancelButton: true,
            preConfirm: (name) => {
                if (!name) {Swal.showValidationMessage('Nama TPS tidak boleh kosong!');}
                return name;
            }
        }).then((result) => {
        if (result.isConfirmed) {
            Swal.fire({
                text: `${result.value} berhasil ditambahkan!`,
                icon: "success",
                showConfirmButton: false,
                timer: 2000
            });
            socket.emit("admin-tps-add-tps", result.value);
        }
        });
        // Swal.fire({
        //     icon: "question",
